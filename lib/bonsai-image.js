...
async function loadPipelineFromSnapshot(
  ...
  try {
    r && r({ phase: `init` });
    let i = p.readJsonResourceOptional
      ? await p.readJsonResourceOptional(n, `scheduler/scheduler_config.json`, m)
      : null;
    i = i ?? {
